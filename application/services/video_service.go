...
func (v *VideoService) Download(bucketName string) error {
	...
	objBucket := bucket.Object(v.Video.FilePath)
	//Reading file
	...
	//This command create a file on the machine
	f, err := os.Create(os.Getenv("LOCAL_STORAGE_PATH") + "/" + v.Video.ID + ".mp4")
	if err != nil {
		return err
	}
	//Store data into created file
	_, err = f.Write(body)
	if err != nil {
		return err
	}
	defer f.Close()
	log.Printf("Video has been stored %v\n", v.Video.ID)
	return nil
}
...
func (v VideoService) Finish() error {
	...
	if err != nil {
		log.Println("Error removing all mp4 ", v.Video.ID)
		return err
	}
	return nil
}
